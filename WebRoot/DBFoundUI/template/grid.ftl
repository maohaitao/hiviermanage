                    ...
                    <#if button.type="add">
                        {
                            ...
                            handler: function () {
                                    <#if  button.beforeAction?? >if (!${button.beforeAction}(${grid.id}, this))
                                    return;</#if>
                            ${grid.id}.
                                addLine();
                    ...
                    <#elseif  button.type="save">
                        {
                            ...
                            action: "${button.action}",
                                <#if button.disabled>disabled: true,</#if>
                            handler: function () {
                                var record_all = ${grid.id}.
                                getSelectionModel().getSelections();
                                var records = [];
                                var j = 0;
                                for (var i = 0; i < record_all.length; i++) {
                                    if (record_all[i].dirty) {
                                        records[j] = record_all[i];
                                        j++;
                                    }
                                }
                                    <#if  button.beforeAction?? >if (!${button.beforeAction}(records,${grid.id}, this)) return;</#if>
    ...
    Ext.onReady(function () {
        ...
        <#if delayRender>
            setTimeout(function () {
                ...
                setTimeout(function () {
                    cmp.getSelectionModel().selectFirstRow();
                }, 100);
            }, 200);
        <#else>
