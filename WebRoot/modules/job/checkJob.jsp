...
<script type="text/javascript">
    ...
    function openAttWindow(id, status) {
        ...
        DBFound.open("att_window", "附件上传", 690, 370, url, function () {
            jobGrid_ds.reload();
        });
    }
...
<d:form id="headerForm" title="作业信息查询" labelWidth="80">
    <d:line columnWidth="0.33">
        ...
        <d:field name="teacher_name" readOnly="true" value="${headerStore[0].teacher_name}" editor="textfield"
                 prompt="布置老师"/>
        <d:field name="class_name" readOnly="true" value="${headerStore[0].class_name}" editor="textfield"
                 prompt="所对班级"/>
    </d:line>
    <d:line columnWidth="0.33">
        <d:field name="status" value="${headerStore[0].status_name}" readOnly="true" editor="textfield" prompt="状态"/>
        <d:field name="course_name" readOnly="true" value="${headerStore[0].course_name}" editor="textfield"
                 prompt="学科选择"/>
        <d:field name="end_time" readOnly="true" value="${headerStore[0].end_time}" editor="datefield" prompt="完成时间"/>
    </d:line>
    <d:line columnWidth="1">
        <d:field name="description" readOnly="true" anchor="95%" value="${headerStore[0].description}" editor="textarea"
                 prompt="描述"/>
    </d:line>
</d:form>
<d:buttonGroup>
...
<d:grid id="jobGrid" height="270" model="job/jobCheck"
        queryUrl="job/jobCheck.query!checkData?header_id=${param.header_id}" autoQuery="true" pagerSize="50">
    <d:toolBar>
    ...
    <d:columns>
        ...
        <d:column name="grade" required="true" editor="combo" options="statusStore" displayField="status_name"
                  valueField="status_code" width="80" prompt="等级"/>
