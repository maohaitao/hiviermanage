...
<script type="text/javascript">
    ...
    function openUpdateWindow(id, status) {
        ...
        if (status == 'NEW') {
            ...
            DBFound.open("update_window", "作业明细", 735, 350, url, function () {
                jobGrid_ds.reload();
            });
        } else if (status == 'CHECK') {
            url = "${basePath}" + "modules/job/checkJob.jsp?header_id=" + id;
            DBFound.open("check_window", "作业明细", 890, 505, url, function () {
                jobGrid_ds.reload();
            });
        } else {
...
<d:initProcedure>
    ...
    <d:dataSet id="course_ds" modelName="fnd/course" queryName="add_combo" loadData="false"
...
<d:form id="queryForm" title="作业工作台" labelWidth="90">
    <d:line columnWidth="0.33">
        ...
        <d:field id="course_cb" name="course_id" options="course_ds" valueField="course_id" displayField="course_name"
                 editor="combo" prompt="课程选择"/>
    </d:line>
    <d:line columnWidth="0.33">
        <d:field name="status_code" editor="combo" options="statusStore" displayField="status_name"
                 valueField="status_code" prompt="状态"/>
...
<d:grid id="jobGrid" title="作业列表" height="350" queryForm="queryForm" model="job/jobHeader" autoQuery="true">
    <d:toolBar>
        ...
        <d:gridButton action="job/jobControl.execute!submit" afterAction="query" icon="DBFoundUI/images/email.png"
                      title="布置"/>
        <d:gridButton action="job/jobControl.execute!check" afterAction="query" icon="DBFoundUI/images/check.png"
                      title="审批"/>
        <d:gridButton action="job/jobControl.execute!finish" afterAction="query" icon="DBFoundUI/images/disk.png"
                      title="完成"/>
    </d:toolBar>
    <d:columns>
